...
- Official parser/rendering path as of 2026-04-22:
  ...
  - `js/hwp-renderer.js`
- The former external WASM bundle is not ChromeHWP's reference engine and is not part of the active runtime path.
...
- Completed:
  ...
  - Kept ChromeHWP's split JS parser modules as the official implementation path.
...
- Remaining work:
  - Re-implement the `incheon-2a.hwpx` page 2 overlap fix in ChromeHWP's own parser/renderer path if it still reproduces there.
