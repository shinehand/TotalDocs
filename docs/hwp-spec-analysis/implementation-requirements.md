...
- 현재 프로젝트의 `HWP 엔진` 이식 작업에서 무엇을 먼저 고쳐야 하는지 명확히 고정한다.
...
1. `js/hwp-wasm-renderer.js`
   - 레이아웃 감사용 필드 노출 범위를 넓힌다.
   - 표/개체/수식/차트의 구조 정보를 브리지로 끌어올린다.
2. `js/app.js`
...
1. `js/hwp-wasm-renderer.js`
   ...
   - raw-preserving 저장 경로가 가능한지 엔진 브리지 한계를 먼저 확인한다.
2. `js/app.js`
