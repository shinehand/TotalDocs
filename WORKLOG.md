...
- 결정:
  - 외부 WASM 실험 저장소는 TotalDocs의 기준 엔진으로 삼지 않는다.
  - TotalDocs의 공식 파서/레이아웃 구현은 저장소 내부 JS 파서와 DOM 렌더러다.
- 수행:
...
- 2026-04-22 결정:
  - 이 구간의 외부 WASM 실험은 TotalDocs 공식 방향에서 제외한다.
  - 이후 레이아웃 구현은 TotalDocs 내부 JS 파서/DOM 렌더러 기준으로 진행한다.
...
- 감사 도구 보강:
  ...
  - 한컴 crop과 Chrome 페이지의 종횡비가 크게 어긋나면 `capture-review`로 표시하는 `captureQuality` 진단 추가.
- 최신 감사:
  - `node scripts/verify_samples.mjs` 통과: 대표 5개 문서 모두 한컴 기준 쪽수 일치.
  - `HANCOM_PAGE_AUDIT_IDS=incheon-2a ... node scripts/capture_hancom_page_audit.mjs` 및 `python3 scripts/build_hancom_page_audit.py ...` 통과.
